...
export const POSScreen: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4.5rem)] overflow-hidden relative pb-16">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden glass-panel rounded-3xl border border-slate-200 dark:border-slate-800 p-2.5 sm:p-4">
        
        {/* Top Controls: Compact Search + Hold Cart Tabs */}
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2 mb-2">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl ps-9 pe-3 py-1.5 text-xs text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-emerald-500"
            />
          </div>

          {/* Compact Hold Cart Switcher Tabs */}
          <div className="flex items-center justify-center bg-slate-100 dark:bg-slate-950/80 p-0.5 rounded-xl border border-slate-200 dark:border-slate-800">
        ...
        {/* Compact Category Horizontal Filter Chips */}
      ...
      <div className="fixed bottom-16 sm:bottom-3 start-3 end-3 sm:start-4 sm:end-4 z-30 max-w-4xl mx-auto">
        <div
          ...
          className="glass-panel p-2 sm:p-3 rounded-2xl border border-emerald-500/50 shadow-xl shadow-emerald-500/10 flex items-center justify-between gap-2 cursor-pointer group hover:border-emerald-500 transition-all"
        >
          {/* Left / Info side: BIGGEST AREA FOR TOTAL AMOUNT */}
          <div className="flex-1 min-w-0 flex items-center gap-2.5">
            <div className="relative w-10 h-10 rounded-xl bg-emerald-600 dark:bg-emerald-500 flex items-center justify-center text-white dark:text-slate-950 font-bold shadow-md shadow-emerald-500/30 group-hover:scale-105 transition-transform flex-shrink-0">
              <ShoppingBag className="w-5 h-5" />
              {activeCartItemCount > 0 && (
                <span className="absolute -top-1.5 -end-1.5 bg-rose-500 text-white text-[9px] font-black w-4.5 h-4.5 rounded-full flex items-center justify-center border-2 border-white dark:border-slate-950">
                  {activeCartItemCount}
                </span>
              )}
            </div>
            ...
            <div className="min-w-0 flex-1">
              <span className="text-[10px] text-slate-500 dark:text-slate-400 font-bold block flex items-center gap-1">
                <span>{activeCartItemCount > 0 ? `${activeCartItemCount} عناصر بالسلة` : 'السلة فارغة'}</span>
                <ChevronUp className="w-3 h-3 text-emerald-600 dark:text-emerald-400 group-hover:-translate-y-0.5 transition-transform" />
              </span>
              <span className="text-lg sm:text-2xl font-black text-emerald-600 dark:text-emerald-400 font-mono tracking-tight dir-ltr block truncate">
                {formatAmount(activeCartTotal)}
              </span>
            </div>
          </div>

          {/* Right side: Compact Quick Action Buttons */}
          <div className="flex items-center gap-1.5 flex-shrink-0" onClick={(e) => e.stopPropagation()}>
            <button
              disabled={activeCart.length === 0}
              onClick={handleCashCheckout}
              className="px-2.5 py-2 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 disabled:opacity-40 disabled:cursor-not-allowed text-white font-black text-[11px] shadow-sm flex items-center gap-1 transition-all active:scale-95 whitespace-nowrap"
            >
            ...
            <button
              ...
              className="px-2.5 py-2 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 disabled:opacity-40 disabled:cursor-not-allowed text-white dark:text-slate-950 font-black text-[11px] shadow-sm flex items-center gap-1 transition-all active:scale-95 whitespace-nowrap"
            >
